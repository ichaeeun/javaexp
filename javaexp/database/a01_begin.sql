...
1.DBMS(database management system): 
	연관성 있는 데이터들의 집합 + 이들 데이터를 효율적으로 응용하기 위해 
	구성된 소프트웨어들의 집합
	- 데이터와 응용프로그램의 중간에서 응용프로그램이 요구하는 대로 데이터를 정의하고,
		읽고, 쓰고, 갱신하는 등의 데이터를 조작하고 이들을 효율적으로 관리하는 
		프로그램들을 지칭한다. 
2. RDBMS(relation database management system) 
	1) 특징
		...
		- 비절차적(non-procedural) 언어인 sql지원 - sql을 통한 join으로 데이터 관계내용을 
												가져올 수 있다. 
		- 시스템 카탈로그 지원(data dictionary) - 데이터 참조에 필요한 위치 정보를 관리 
...
4. sql언어의 특징	
	1) 비절차적 언어 
		...
		sql언어: 조건을 만족하는 데이터를 집합 단위로 한꺼번에 처리 
	2) 대화식으로 사용되거나 응용 프로그램에 삽입하여 사용 
	
5. sql 명령문의 종류 
	1) 질의어(DQL: data query language): 데이터를 검색 select 
	2) 데이터 조작어(DML: data manipulation language):
		데이터를 입력/수정/삭제 insert, update, delete
		ex) insert 테이블(컬럼1,컬럼2) values(데이터1, 데이터2...);	
	3) 데이터 정의어(DDL: data definition language): 데이터베이스 객체 생성
		객체 변경(alter), 객체 삭제(drop), 객체 이름 변경(rename), 
		데이터 및 저장 공간 삭제(truncate)
	4) 트랜젝션 처리어(TCL: transaction control language): 트랜젝션의 정상적인 종료 
		처리(commit), 트랜잭션 취소(rollback), 트랜잭션내의 임시 저장점 설정(savepoint)
		ex) 사용자1 scott계정 접근 데이터 조작어 insert를 하면, 현재 사용자1에서 입력 되어 있지만, 
			다른 사용자2로 해당 계정에 접근하면 입력되어 있지 않다. 이때, 사용자1이 입력 후, 
			commit하는 순간 다른 사용자로 접근해서 해당 테이블에 데이터가 입력된 것을 확인할 수 있다.
	5) 데이터 제어어(DCL: data controll language): 데이터 객체에 대한 접근 권한 부여 
		(grant), 데이터베이스 객체에 대한 접근권한 취소(revoke)
6. sql을 잘 활용하기 위한 tip
	...
	7) 1000줄의 절차적 프로그램을 10여줄로 처리가능 
	8) 1000초의 수행 속도를 약간의 변경으로 1초로도 향상가능 
