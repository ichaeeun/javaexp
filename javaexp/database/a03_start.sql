 ...
 */
SELECT *	-- 한줄주석   *: 전케 컬럼 
FROM emp;	-- ctrl + enter 로 마지막 명령 

-- emp 테이블 초기에 선언 컬럼이 순서대로 출력을 한다. 
SELECT empno,job,sal,deptno --select 컬럼명1,컬럼명2,...
FROM emp;					--
--ex) dept 테이블의 전체 컬럼내용을 출력하세요. 
SELECT *
FROM dept;
--ex2) emp 테이블의 empno,sal,ename을 출력하세요 
SELECT empno,sal,ename
FROM emp;
--저장 file->save as->workspace database폴더에 저장. 
/*
 3. 컬럼명 별칭처리하기.. 
 select 컬럼명 as 별칭명, 컬렴명 별칭명 
 	1) 기본 테이블에서 지정한 컬럼명이외에 다른 이름으로 컬럼명을 위 형식으로 처리하면,
 		출력할 수 있다. 
 	2) 컬럼명 안에 공백이나 특수문자(#,@)를 활용할때는 "별칭" 형식으로 
 		쌍따옴표 사이에 별칭을 지정하면 된다. 
...
FROM emp; 
-- ex)사원테이블에 사원 번호(empno), 사 원 명(ename), 관리자 번호(mgr), 
--		급 여(sal)로 별칭으로 데이터를 표현하세요 
SELECT empno "사원 번호", ename "사 원 명", mgr "관리자 번호", sal "급 여"
FROM emp; 
/*
 # 데이터의 컬럼간의 문자열 연결과 산술연산 처리 
 1. 문자열형 컬럼은 문자열컬럼1 || 문자열컬럼2 형식으로 연결하면, 
	해당 컬럼에 있는 데이터가 연결되어 출력된다. 
...
FROM emp;
SELECT ename || '의 사원번호는 ' || empno || '이고, 10%인상된급여는 ' || 
		(sal+(sal*0.1))||'만원이다' "급여정보"
FROM emp;
--ex)ename(empno)의 월급여는 sal/13 입니다. 를 한문장으로 출력하세요 
-- round(): 소숫점이하 절삭 (함수) 
SELECT ename || '(' || empno || ')의 월급여는 ' || round(sal/13) || '만원 입니다.' show
FROM emp;
--ex) ename 님의 이달 보너스는 sal의 deptno %기준으로 sal*(deptno/100)만원이다. 
SELECT ename || '님의 이달 보너스는 ' || sal ||'의 '|| deptno || '% 기준으로 ' ||
...
FROM emp
ORDER BY deptno; 
-- 각 deptno를 데이터별로 그룹지어 최고값 sal를 출력 
SELECT deptno, max(sal) 최고, min(sal) 최저, round(avg(sal)) 평균 
FROM EMP 
GROUP BY deptno; 
--ex1) emp 테이블 mgr을 중복을 배제하고 출력하세요 
SELECT DISTINCT mgr 
FROM emp;
--ex2) emp 테이블 deptno가 10인 데이터를 출력하세요 
SELECT *
FROM EMP 
where deptno=10; --database에서는 =, >=, <, >, != 
--ex3) emp 테이블 job별 최고 연봉(sal)을 출력하세요
SELECT job, max(sal) 최고연봉
FROM EMP 
GROUP BY job;
--ex4) emp테이블을 sal별로 정렬하여 출력하세요 
SELECT *
FROM EMP 
ORDER BY sal;
 ...
 */
 ...
 4. 문자와 날짜 타입의 상수값을 작은 따옴표(')로 묶어서 표현하고 
 	숫자는 그대로 사용한다. 
 */
	SELECT * FROM emp
	WHERE job = 'SALESMAN';
...
--ex) job이 'SALESMAN'인 데이터를 출력하되, 컬럼은 ENMAE, JOB, SAL를 선택 
	SELECT ename, job, sal
	FROM emp 
	WHERE job ='SALESMAN';
--ex) 부서번호(deptno)가 20이고, sal가 4000미만인 데이터를 출력하세요 
	SELECT *
	FROM emp 
	WHERE deptno=20 AND sal<4000;
--ex) 연봉(sal)이 2000에서 3000사이 이거나, 직책(job)이 'SALESMAN'인 사원의
...
SELECT sal, comm 
FROM emp 
WHERE comm IS NULL; -- comm = null:(버그로 정상적인 데이터를 가져오지 못한다.)
SELECT sal, comm 
FROM emp 
...
-- ex3) comm이 null이 아닌 데이터를 ename과 sal의 comm의 합산한 급여로 출력.
SELECT ename,sal, comm, sal+comm 합산 --comm이 null이면 합산처리에 문제가 있다. 
FROM emp 
WHERE comm IS NOT NULL; -- null이 아닌 경우만 호출 
