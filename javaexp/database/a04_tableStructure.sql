/*
 ...
 1. 개요 
 	1) 데이터베이스에 저장되는 데이터는 문자, 숫자, 날짜, 이미지 등과 같은 	
 		다양한 형식으로 표현된다. 
 	2) 실무에서 컬럼에 대한 데이터 타입이나 길이의 결정은 데이터베이스 설계단계에서 
 		신중하게 고려해야 하는 중요한 사항 
 	3) 데이터베이스 설계 단계에서 잘못 선정된 데이터타입은 응용 프로그램 
 ...
 2. 오라클에서 지원되는 데이터 타입의 종류 
 	...
 	5) nvarchar2(size) : 국가별 문자 집합에 따른 size크기의 문자/바이트 
 		문자 데이터 타입 
 	6) rowid : 테이블내 행의 고유 주소를 가지는 64진수 문자타입 
 		행단 6바이트 또는 10바이트
...
INSERT INTO MEMBER values('himan','7777','9112342123456',1000,'관리자');

-- ex) 프로야구 선수 팀 정보를 입력하려고 한다. 구글에 프로야구 순위 확인하고, 
--		해당 데이터를 입력하기 위한테이블을 만들고, 데이터를 입력한 후, 조회하세요 
...
FROM baseteam;
INSERT INTO baseteam values('NC',83,55,6);
INSERT INTO baseteam values('KT',81,62,1);
INSERT INTO baseteam values('LG',79,61,4);
INSERT INTO baseteam values('두산',79,61,4);
